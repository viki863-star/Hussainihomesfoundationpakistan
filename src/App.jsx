...
/* ---- Expanding Floating Action Buttons (WhatsApp + Donate) ---- */
function FloatingActions({ donateLabel, donateSub, whatsappSub }) {
  ...
  return (
    <div className="fabs" ref={rootRef}>
      {/* WhatsApp */}
      <div className={`fab fab-whatsapp${open === 'whatsapp' ? ' open' : ''}`}>
        <a
          className="fab-card"
          href="https://wa.me/923034030009"
          target="_blank"
          rel="noopener noreferrer"
          tabIndex={open === 'whatsapp' ? 0 : -1}
        >
          <span className="fab-card-inner">
            <span className="fab-label">WhatsApp</span>
            <span className="fab-sub">{whatsappSub}</span>
          </span>
        </a>
        <button
          type="button"
      ...
      {/* Donate */}
      <div className={`fab fab-donate${open === 'donate' ? ' open' : ''}`}>
        <a
          className="fab-card"
          href="#donate"
          tabIndex={open === 'donate' ? 0 : -1}
        >
          <span className="fab-card-inner">
            <span className="fab-label">{donateLabel}</span>
            <span className="fab-sub">{donateSub}</span>
          </span>
        </a>
        <button
          type="button"
          ...
          aria-expanded={open === 'donate'}
          onClick={() => toggle('donate')}
        >
          <span className="fab-icon-glyph" aria-hidden="true">♥</span>
          <span className="fab-ripple" aria-hidden="true" />
        </button>
      </div>
    </div>
